...
class TextSummarizer:
    ...
    def __init__(self, model_name_or_path: str = None, device: Optional[str] = None, model_name: str = None):
        """
    ...
    def train(
        ...
        history = {"train_loss": [], "eval_loss": [], "rouge_scores": []}
    ...
    def evaluate(self, dataloader, tokenizer=None, compute_rouge=False):
        ...
        if compute_rouge and tokenizer:
            try:
                ...
                print(
                    "Warning: rouge_score package not found. ROUGE metrics not computed."
                )
        ...
        **generate_kwargs,
        ...
        for i in range(0, len(texts), batch_size):
            ...
            summary_ids = self.summarize(
                inputs, max_length=max_length, min_length=min_length, **generate_kwargs
            )
    ...
    def load(cls, path: str, device: Optional[str] = None):
        ...
        """
        # Create instance
        instance = cls(model_name_or_path=path, device=device)
...
class ExtractiveSummarizer:
    ...
    def __init__(self, method: str = "tfidf"):
        ...
        Args:
            method: Summarization method ('tfidf', 'textrank', 'lexrank', 'lsa', 'luhn')
        """
    ...
    def summarize(
        self, text: str, ratio: float = 0.3, max_sentences: Optional[int] = None
    ):
        """
        ...
        if self.method == "tfidf":
            ...
            ranked_indices = self._lsa_ranking(sentences)
        elif self.method == "luhn":
            ranked_indices = self._luhn_ranking(sentences)
        else:
    ...
    def _lsa_ranking(self, sentences: List[str]) -> List[int]:
        """
        Rank sentences using Latent Semantic Analysis (LSA).
        ...
        n_components = min(len(sentences) - 1, 3)  # Use at least 1 component, at most 3
        svd = TruncatedSVD(n_components=n_components, random_state=42)
        svd.fit(tfidf_matrix)
        
        # Get document-topic matrix
        sentence_topics = svd.transform(tfidf_matrix)
        
        # Calculate sentence scores as the sum of squared values
        scores = np.sum(np.square(sentence_topics), axis=1)
    ...
    def _luhn_ranking(self, sentences: List[str]) -> List[int]:
        """
        Rank sentences using Luhn's algorithm.
        
        Luhn's algorithm ranks sentences based on important words frequency.
        ...
        # Create TF-IDF matrix for word importance
        vectorizer = TfidfVectorizer(stop_words="english")
        tfidf_matrix = vectorizer.fit_transform(sentences)
        
        # Get vocabulary and inverse index
        vocabulary = vectorizer.get_feature_names_out()
        
        # Find significant words (words with high TF-IDF values)
        word_tfidf = np.asarray(tfidf_matrix.mean(axis=0)).ravel()
        significant_word_indices = np.where(word_tfidf > np.mean(word_tfidf))[0]
        significant_words = set([vocabulary[i] for i in significant_word_indices])
        
        # Calculate sentence scores based on clusters of significant words
        scores = []
        for sentence in sentences:
            ...
            sig_word_positions = [i for i, word in enumerate(words) if word.lower() in significant_words]
            
            if not sig_word_positions:
                scores.append(0)
                continue
                
            # Find clusters
            clusters = []
            cluster = [sig_word_positions[0]]
            
            for pos in sig_word_positions[1:]:
                if pos - cluster[-1] < 4:  # Words less than 4 positions apart are in same cluster
                    cluster.append(pos)
                else:
                    if len(cluster) > 1:  # Only consider clusters with at least 2 significant words
            ...
            # Calculate score based on the best cluster
            if clusters:
                best_cluster = max(clusters, key=len)
                score = (len(best_cluster) ** 2) / len(words)
            else:
                score = 0
...
class ControlledSummarizer:
    ...
    def summarize(
        ...
        inputs = tokenizer(
            prompt, padding=True, truncation=True, max_length=1024, return_tensors="pt"
        )
...
def create_abstractive_summarizer(model_name: str, **kwargs):
    ...
    Returns:
        TextSummarizer instance
    """
    return TextSummarizer(model_name_or_path=model_name, **kwargs)
...
def create_extractive_summarizer(method: str = "tfidf", **kwargs):
    ...
    """
    return ExtractiveSummarizer(method=method, **kwargs)
...
def create_controlled_summarizer(model_name: str, **kwargs):
    ...
    """
    return ControlledSummarizer(model_name_or_path=model_name, **kwargs)
