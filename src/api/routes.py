...
class ClassificationRequest(BaseModel):
    ...
    text: Union[str, List[str]] = Field(..., description="Text to classify, can be a single string or list of strings")
    model_name: Optional[str] = Field(None, description="Model name to use for classification")
...
class ClassificationResponse(BaseModel):
    ...
    predictions: List[Union[str, int]] = Field(..., description="List of predicted classes")
    probabilities: Optional[List[Dict[str, float]]] = Field(None, description="List of class probabilities")
...
class NERRequest(BaseModel):
    ...
    align_to_words: bool = Field(True, description="Whether to align entity spans to word boundaries")
...
class SentimentRequest(BaseModel):
    ...
    model_name: Optional[str] = Field(None, description="Model name to use for sentiment analysis")
    return_probabilities: bool = Field(False, description="Whether to return sentiment probabilities")
    aspects: Optional[List[str]] = Field(None, description="Specific aspects to analyze (for aspect-based sentiment)")

class SentimentResponse(BaseModel):
    """Response model for sentiment analysis."""
    sentiments: List[Union[str, Dict[str, str]]] = Field(..., description="List of sentiments or aspect-sentiment mappings")
    probabilities: Optional[List[Dict[str, float]]] = Field(None, description="List of sentiment probabilities")
    model_name: str = Field(..., description="Model used for the prediction")
...
class SummarizationRequest(BaseModel):
    ...
    model_name: Optional[str] = Field(None, description="Model name to use for summarization")
    ...
    method: Literal["abstractive", "extractive"] = Field("abstractive", description="Summarization method")
    length: Literal["short", "medium", "long"] = Field("medium", description="Summary length preset")
...
class ModelInfo(BaseModel):
    ...
    languages: Optional[List[str]] = Field(None, description="Languages supported by the model")
...
async def classify_text(request: ClassificationRequest):
    ...
    try:
        ...
        # Get predictions
        predictions = model.predict(inputs, return_probabilities=request.return_probabilities)
...
async def recognize_entities(request: NERRequest):
    ...
    try:
        ...
        for text_entities in entities_list:
            ...
            if isinstance(text_entities, list) and all(isinstance(e, dict) for e in text_entities):
                # Entities are already in the correct format
                entities = [Entity(**e) for e in text_entities]
            elif isinstance(text_entities, list) and all(isinstance(e, str) for e in text_entities):
                # Only token-level labels, create simple entities
                entities = [Entity(entity="", label=label, start=None, end=None) for label in text_entities]
            else:
                # Unexpected format
                entities = []
        ...
        if request.aspects:
            ...
            if aspect_model:
                ...
                return SentimentResponse(
                    ...
                    model_name=model_name + "_absa"
                )
            else:
                # Fall back to regular sentiment analysis
                logger.warning(f"Aspect-based model not found for {model_name}, falling back to standard model")
