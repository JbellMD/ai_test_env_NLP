...
def plot_confusion_matrix(y_true: Union[List[int], np.ndarray],
...
def plot_attention(attention_weights: np.ndarray, tokens: List[str],
    ...
    # Plot attention weights
    sns.heatmap(attention_weights, annot=False, cmap=cmap, xticklabels=tokens, yticklabels=tokens, square=True)
...
def plot_token_classification_results(tokens: List[str], true_labels: List[str], 
    ...
    for i, (token, true_label, pred_label) in enumerate(zip(tokens, true_labels, pred_labels)):
        ...
        # Plot true label above
        ax.text(pos[0], pos[1] + 0.2, true_label, ha='center', va='center', 
               fontsize=10, color='black', bbox=dict(facecolor=colors[true_label], alpha=0.5))
        
        # Plot predicted label below
        ax.text(pos[0], pos[1] - 0.2, pred_label, ha='center', va='center', 
               fontsize=10, color='black', bbox=dict(facecolor=colors[pred_label], alpha=0.5))
    
    # Add legend
    handles = [plt.Rectangle((0, 0), 1, 1, color=colors[label]) for label in all_labels]
    ax.legend(handles, all_labels, loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol=min(6, len(all_labels)))
...
def plot_classification_metrics(y_true: Union[List[int], np.ndarray],
                              y_pred: Union[List[int], np.ndarray],
    ...
    if y_proba is not None:
        ...
        # For multi-class classification
            ...
            for i in range(n_classes):
                ...
                roc_auc = auc(fpr, tpr)
                class_name = class_names[i] if class_names else f"Class {i}"
                axes[2].plot(fpr, tpr, linewidth=2, label=f"{class_name} (AUC = {roc_auc:.2f})")
        ...
        for i in range(1, 3):
            axes[i].text(0.5, 0.5, 'Probabilities required\nfor this plot',
...
def plot_summarization_results(texts: List[str], summaries: List[str], 
    ...
    for i in range(n_examples):
        ...
        axes[i].text(0.05, 0.95, display_text, va='top', ha='left',
...
def plot_word_importances(text: str, word_importances: Union[List[float], np.ndarray], 
                        figsize: Tuple[int, int] = (15, 5),
                        cmap: str = 'coolwarm',
                        save_path: Optional[str] = None):
    """
    ...
    assert len(words) == len(word_importances), "Number of words and importances must match"
    
    # Normalize importances to [0, 1] for coloring
    norm_importances = (word_importances - np.min(word_importances)) / (np.max(word_importances) - np.min(word_importances) + 1e-8)
    ...
    for i, (word, importance) in enumerate(zip(words, norm_importances)):
        ...
        t = ax.text(x_pos, 0.5, word + ' ', fontsize=14, color='black',
...
def visualize_embeddings(embeddings: np.ndarray, labels: Optional[List[Any]] = None, 
                       method: str = 'tsne', 
                       figsize: Tuple[int, int] = (10, 8),
                       save_path: Optional[str] = None):
    """
    ...
    if labels is not None:
        ...
        scatter = plt.scatter(embeddings_2d[:, 0], embeddings_2d[:, 1], c=numeric_labels, 
                           cmap='tab10', alpha=0.8, s=50)
        
        # Add legend
        if len(unique_labels) <= 10:  # Only show legend for a reasonable number of classes
