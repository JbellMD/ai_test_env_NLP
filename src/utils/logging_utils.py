...
def setup_file_logging(logger: logging.Logger, log_dir: str, filename: Optional[str] = None) -> str:
    """
...
class JsonLogger:
    ...
    def log_artifact(self, artifact_path: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        """
...
class ExperimentTracker:
    ...
    def __init__(self, experiment_name: str, run_id: Optional[str] = None, 
        ...
        self.logger.info(f"Initialized experiment: {experiment_name}, run: {self.run_id}")
    ...
    def save_artifact(self, data: Any, name: str, artifact_type: str = 'general') -> str:
        """
        Save an artifact.
        ...
        with open(artifact_path, 'w' if artifact_type in ('json', 'text') else 'wb', encoding='utf-8' if artifact_type in ('json', 'text') else None) as f:
            save_fn(data, f)
        
        self.logger.info(f"Saved artifact '{name}' to {artifact_path}")
        
        # Log as artifact
        self.json_logger.log_artifact(artifact_path, {'type': artifact_type, 'name': name})
