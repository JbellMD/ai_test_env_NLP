...
def accuracy(preds, labels):
    ...
    # Handle empty inputs - convention: accuracy of empty set is 0
...
def rouge_metrics(predictions, references, rouge_types=None):
    ...
    if not predictions or not references:
        raise ValueError("Empty inputs: ROUGE metrics require non-empty texts")
    ...
    # Calculate averages
    results = {
        rouge_type: sum(scores[rouge_type]) / len(scores[rouge_type]) if scores[rouge_type] else 0.0
        for rouge_type in rouge_types
    }
...
def bleu_score(predictions, references, max_order=4, smooth=False):
    ...
    if isinstance(references[0][0], str):  # references is list of list of strings
        ...
        references = [
            [nltk.word_tokenize(ref.lower()) for ref in refs] for refs in references
        ]
    ...
    return corpus_bleu(
        references, predictions, weights=weights, smoothing_function=smoothing_function
    )
    ...
    if task.lower() == "classification":
        ...
        if ROUGE_AVAILABLE and isinstance(preds[0], str):
            metrics = rouge_metrics(
                preds, labels, rouge_types=kwargs.get("rouge_types")
            )
        else:
            metrics = (
                {"accuracy": accuracy(preds, labels)}
        ...
        if isinstance(preds[0], (int, np.integer)) and isinstance(
            labels[0], (int, np.integer)
        ):
            metrics = {"accuracy": accuracy(preds, labels)}
        else:
            metrics = {}
...
def exact_match(preds, labels):
    ...
    if isinstance(preds[0], str):
        ...
        return sum(p == l for p, l in zip(normalized_preds, normalized_labels)) / len(
            preds
        )
    else:
