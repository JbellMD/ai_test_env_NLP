...
class BaseTrainer:
    ...
    def train(self, num_epochs: int, **kwargs):
        ...
        if self.save_path and (
            self.save_every is None or self.global_step % self.save_every != 0
        ):
    ...
    def train_epoch(self, epoch: int, **kwargs):
        ...
        for step, batch in enumerate(
            tqdm(self.train_dataloader, desc=f"Epoch {epoch+1}")
        ):
            ...
            if (
                ...
                self.save_model(
                    os.path.join(self.save_path, f"model_step_{self.global_step}")
                )
    ...
    def save_model(self, path: str):
        ...
        with open(os.path.join(path, "training_history.json"), "w") as f:
            ...
            for k, v in self.history.items():
                json_safe_history[k] = [
                    float(x) if isinstance(x, (np.number, np.ndarray)) else x for x in v
                ]
...
class NERTrainer(BaseTrainer):
    ...
    def calculate_ner_metrics(self, predictions, labels):
        ...
        if hasattr(self.model, "config") and hasattr(self.model.config, "id2label"):
            ...
            str_predictions = [[id2label[p] for p in pred] for pred in predictions]
...
class SummarizationTrainer(BaseTrainer):
    ...
    def __init__(self, *args, tokenizer=None, compute_rouge=True, **kwargs):
        ...
        kwargs["metrics"] = kwargs.get(
            "metrics", ["loss", "rouge1", "rouge2", "rougeL"]
        )
        super().__init__(*args, **kwargs)
    ...
    def calculate_rouge_metrics(self, predictions, references):
        ...
        try:
            ...
            # Average scores
            metrics["rouge1"] = sum(rouge_scores["rouge1"]) / len(
                rouge_scores["rouge1"]
            )
            metrics["rouge2"] = sum(rouge_scores["rouge2"]) / len(
                rouge_scores["rouge2"]
            )
            metrics["rougeL"] = sum(rouge_scores["rougeL"]) / len(
                rouge_scores["rougeL"]
            )
