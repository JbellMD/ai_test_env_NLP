...
from src.data.data_loader import (
    NLPDatasetLoader,
    get_ner_loader,
    get_summarization_loader,
    get_text_classification_loader,
)
...
from src.models.summarizer import (
    create_abstractive_summarizer,
    create_extractive_summarizer,
)
from src.training.trainer import create_trainer
from src.utils.logging_utils import ExperimentTracker, get_logger
...
def parse_args():
    ...
    # Training parameters
    parser.add_argument(
        "--config_file", type=str, default=None, help="Path to model configuration file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_labels", type=int, default=None, help="Number of labels/classes"
    )
    ...
    parser.add_argument(
        "--remove_punctuation", action="store_true", help="Remove punctuation"
    )
    parser.add_argument(
        "--remove_stopwords", action="store_true", help="Remove stopwords"
    )
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Generate visualizations of training"
    )
...
def train_model(args):
    ...
    experiment_name = (
        args.experiment_name or f"{args.task}_{args.model_name.split('/')[-1]}"
    )
    experiment = ExperimentTracker(
        experiment_name=experiment_name, base_dir=args.output_dir
    )
    ...
    if os.path.isfile(args.dataset):
        ...
        if args.dataset.endswith(".csv"):
            ...
            dataset = dataset_loader.load_from_json(
                args.dataset, text_key=args.text_column, label_key=args.label_column
            )
        else:
        ...
        dataset = dataset_loader.load_huggingface_dataset(
            args.dataset, text_column=args.text_column, label_column=args.label_column
        )
    ...
    if args.task == "classification":
        model = create_classifier(model_name=args.model_name, num_labels=num_labels)
    elif args.task == "ner":
        model = create_ner_model(model_name=args.model_name, num_labels=num_labels)
    elif args.task == "sentiment":
        model = create_sentiment_analyzer(
            model_name=args.model_name, num_labels=num_labels
        )
    elif args.task == "summarization":
        model = create_abstractive_summarizer(model_name=args.model_name)
    ...
    if args.visualize:
        ...
        history_plot_path = os.path.join(
            experiment.artifacts_dir, "training_history.png"
        )
