...
def evaluate_model(args):
    ...
    try:
        ...
        if os.path.exists(os.path.join(args.model_path, "tokenizer")):
            ...
            tokenizer = None
            if args.task != "summarization" or not isinstance(
                model, ExtractiveSummarizer
            ):
                logger.warning(
                    "Tokenizer not found. Some functionality may be limited."
                )
    ...
    if tokenizer:
        ...
        dataset_loader = NLPDatasetLoader(
            preprocessor=preprocessor, task_type=args.task
        )
        ...
        if args.task == "classification" or args.task == "sentiment":
            ...
            if hasattr(model, "evaluate") and tokenizer:
                ...
                if args.visualize and tokenizer:
                    ...
                    if hasattr(model, "id2label"):
                        class_names = [
                            model.id2label[i] for i in range(len(model.id2label))
                        ]
                    ...
                    metrics_path = os.path.join(
                        args.output_dir, "classification_metrics.png"
                    )
                    plot_classification_metrics(
                        y_true=all_labels,
                        y_pred=all_preds,
                ...
                # Manual evaluation
                logger.warning(
                    "Model evaluate method not available. Using manual evaluation."
                )
            ...
            if hasattr(model, "evaluate"):
                ...
                if args.detailed_report and tokenizer:
                    ...
                    for batch_idx, batch in enumerate(dataloader):
                        ...
                        for i, (tokens, preds, labels) in enumerate(
                            zip(token_batch, predictions, batch["labels"])
                        ):
                            pred_list = []
                            label_list = []
                            token_list = []

                            for j, (token, pred, label) in enumerate(
                                zip(tokens, preds, labels)
                            ):
                    ...
                    if args.visualize and examples:
                        for i, example in enumerate(examples):
                            ...
                            viz_path = os.path.join(
                                args.output_dir, f"ner_example_{i}.png"
                            )
                            plot_token_classification_results(
            ...
            if args.summarization_metrics and tokenizer:
                ...
                if isinstance(model, TextSummarizer) and tokenizer:
                    ...
                    summaries = [
                        model.summarize(text, ratio=0.3)
                        for text in texts[: min(len(texts), 50)]
                    ]
                else:
                ...
                if references and len(references) == len(summaries):
                    ...
                    results_path = os.path.join(
                        args.output_dir, "summarization_results.json"
                    )
                    with open(results_path, "w") as f:
                    ...
                    if args.detailed_report:
                        examples = []
                        for i, (text, summary, reference) in enumerate(
                            zip(texts, summaries, references)
                        ):
                        ...
                        examples_path = os.path.join(
                            args.output_dir, "summarization_examples.json"
                        )
                        with open(examples_path, "w") as f:
                            json.dump(examples, f, indent=2)
                else:
                ...
                logger.info(
                    "Summarization evaluation requires --summarization_metrics flag"
                )
    else:
